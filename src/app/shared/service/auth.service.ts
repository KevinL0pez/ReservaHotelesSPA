...
export class AuthService {
    ...
    public registerNewUser(newUser: IUserRegister): Observable<IResponse> {
        return this.httpClient.post<IResponse>(`http://localhost:8080/users/createuser`, newUser);
    }

    public loginUser(user: ILoginUser): Observable<IResponse> {
        return this.httpClient.post<IResponse>(`http://localhost:8080/auth/login`, user);
    }
